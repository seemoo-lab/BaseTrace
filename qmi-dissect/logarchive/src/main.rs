...
fn main() {
    ...
    parse_log_archive(&args.input, &args.output, true);
}

// Parse a provided directory path. Currently, expect the path to follow macOS log collect structure
fn parse_log_archive(path: &str, output_path: &str, high_volume_speedup: bool) {
...
fn filter_cellular(log_data: &LogData) -> bool {
    ...
    return log_data.message.starts_with(CONTENT);
}
